...
export class RouteService implements IRouteService {
  ...
  getRouteMap(): Record<string, string[]> {
    ...
    try {
      ...
      routeMap["system"].push(`GET ${ApiPath.DOCUMENTATION}`);
      routeMap["system"].push(
        `GET ${this.fastify.config.API_VERSION}${ApiPath.HEALTH}`,
      );
  ...
  logAllRoutes(): void {
    try {
      ...
      this.fastify.log.info(
        `API Documentation: ${baseUrl}${ApiPath.DOCUMENTATION}`,
      );
      this.fastify.log.info(
        `Health Check: ${baseUrl}/api/${this.fastify.config.API_VERSION}${ApiPath.HEALTH}`,
      );
    } catch (error) {
