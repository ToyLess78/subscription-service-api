import type { IDatabaseClient, DatabaseConnectionStatus } from "./database.interface"
...
export class PrismaService implements IDatabaseClient {
  ...
  constructor(logger: { info: (msg: string) => void; error: (msg: string, err?: Error) => void }) {
    this.logger = logger
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes("Cannot find module")) {
          ...
          try {
            ...
            throw new Error("Failed to generate Prisma client. Please run 'npx prisma generate' manually.")
          }
        } else {
          throw error
        }
  ...
  async runMigrations(): Promise<void> {
    try {
      ...
      throw new DatabaseError("Failed to run database migrations", { cause: err })
    }
  }
